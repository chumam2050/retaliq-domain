...
CONFIG=${CONFIG:-/etc/retaliq-domain.json}
# ensure BIN is absolute path to the helper binary, even when script is run via relative path
BIN=${BIN:-$(dirname "$(readlink -f "$0")")/build/retaliq-domain}
SERVICE_NAME=retaliq-domain.service

check_jq() {
    if ! command -v jq >/dev/null 2>&1; then
        echo "jq is required and not installed. attempting to install..."
        if command -v apt-get >/dev/null 2>&1; then
            apt-get update
            apt-get install -y jq
        fi
        if ! command -v jq >/dev/null 2>&1; then
            echo "jq still not available; please install it and rerun."
            exit 1
        fi
    fi
}
...
ensure_config() {
    ...
    if [ ! -f "$CONFIG" ]; then
        jq -n \
            --arg key "$(openssl rand -base64 24 | tr '+/' '-_' | tr -d '=')" \
            --argjson ips '["127.0.0.1"]' \
            '{api_key: $key, allowed_ips: $ips}' >"$CONFIG"
        chmod 600 "$CONFIG"
...
show_info() {
    [ -f "$CONFIG" ] || return
    echo "api_key: $(jq -r '.api_key' "$CONFIG")"
    echo "allowed_ips: $(jq -r '.allowed_ips | join(",")' "$CONFIG")"
    echo "port: 8888"
}
...
regen_key() {
    ...
    jq --arg k "$newkey" '.api_key=$k' "$CONFIG" >"$CONFIG.tmp" && mv "$CONFIG.tmp" "$CONFIG"
...
set_allowed_ip_hosts() {
    ...
    local json
    json=$(printf '%s\n' "${all_ips[@]}" | jq -R . | jq -s .)
    jq --argjson a "$json" '.allowed_ips=$a' "$CONFIG" >"$CONFIG.tmp" && mv "$CONFIG.tmp" "$CONFIG"
